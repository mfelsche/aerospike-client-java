...
public abstract class RWTask implements Runnable {
	...
	protected void doWrite(long keyIdx, boolean multiBin, WritePolicy writePolicy) {
		Key key = new Key(args.namespace, args.setName, MapBenchStuff.getKey(keyStart + keyIdx));
	...
	protected void doIncrement(long keyIdx, int incrValue) {
		...
		try {
			add(new Key(args.namespace, args.setName, MapBenchStuff.getKey(keyStart + keyIdx)), bins);
	...
	protected void doRead(long[] keyIdxs, boolean multiBin) {
		try {
			...
			for (int i = 0; i < keyIdxs.length; i++) {
				keys[i] = new Key(args.namespace, args.setName, MapBenchStuff.getKey(keyStart + keyIdxs[i]));
			}
	...
	protected void doReadBatch(boolean multiBin) {
		...
		for (int i = 0; i < keys.length; i++) {
			...
			keys[i] = new Key(args.namespace, args.setName, MapBenchStuff.getKey(keyStart + keyIdx));
		}
